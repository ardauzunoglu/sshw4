        ...
        for sentence in splitter.split(paragraph):
            ...
            for idx, _ in enumerate(token_ids):
                ...
                if idx + local_window_size >= len(tokens):
                    # have already traversed all of the sentences
                    break
                ...
                if idx+local_window_size+1 < len(token_ids):
                  tokens_subset = token_ids[idx:idx+local_window_size]
                  x_data.append(tokens_subset)
...
class NPLMFirstBlock(nn.Module):
    ...
    def forward(self, inputs):
        ...
        emb_vec = embeddings.view(embeddings.size(0), -1)
        
        transformed_emb = torch.tanh(self.linear(emb_vec))
        normalized_emb = self.layer_norm(transformed_emb)
        final_embeds = self.dropout(normalized_emb)
        # your code ends here
...
class NPLMBlock(nn.Module):
    ...
    def forward(self, inputs):
        ...
        transformed_inputs = transformed_inputs + inputs
        # apply layer normalization
        normalized_inputs = self.layer_norm(transformed_inputs)
        # apply dropout
        final_inputs = self.dropout(normalized_inputs)
        # your code ends here
...
class NPLMFinalBlock(nn.Module):
    ...
    def forward(self, inputs):
        ...
        transformed_emb = self.linear(inputs)
        # apply log_softmax to get log-probabilities (logits)
        log_probs = F.log_softmax(transformed_emb, dim=1)
        # your code ends here
...
class NPLM(nn.Module):
    ...
    def forward(self, inputs):
        ...
        logits = self.first_layer(inputs)
        ...
        # output layer
        log_probs = self.final_layer(logits)
        # your code ends here
